################################################################################
# Logistic Antiviral Efficacy Model — Transmission Reduction
# Bayesian estimation via Stan (HMC/NUTS)
#
# Model: E(t) = Emax / (1 + exp(kappa * (t - tau)))
# All three parameters (Emax, kappa, tau) are freely estimated.
################################################################################
...
obs <- data.frame(
  a     = c(0,    1,    2),
  b     = c(1,    2,    3),
  E_obs = c(0.46, 0.31, 0.20),
  type  = c("interval", "interval", "interval"),
  stringsAsFactors = FALSE
)

# [2] Parameter bounds (uniform prior support)
EMAX_LOWER  <- 0.01
EMAX_UPPER  <- 1.00
KAPPA_LOWER <- 0.01
KAPPA_UPPER <- 10.0
...
# [3] Likelihood measurement error SD
SIGMA <- 0.03
...
data {
  int<lower=0> N_int;
  array[N_int] real a_int;
  array[N_int] real b_int;
  array[N_int] real E_obs_int;

  int<lower=0> N_point;
  array[N_point] real t_point;
  array[N_point] real E_obs_point;
...
parameters {
  real<lower=Emax_lower,  upper=Emax_upper>  Emax;
...
# ============================================================
OUTPUT_DIR <- "output"
CACHE_FILE <- file.path(OUTPUT_DIR, "antiviral_eff_estim_transmission.rds")
...
# ============================================================
cat("\n=============================================================\n")
cat(" Posterior Summary — Transmission Reduction\n")
cat("=============================================================\n")
...
# Observed points
obs_plot <- obs %>%
  mutate(t_plot = ifelse(type == "interval", (a + b) / 2, a))

# --------------------------------------------------------------
# Figure 1 — Posterior fit (blue theme for transmission reduction)
# --------------------------------------------------------------
p_fit <- ggplot() +
  ...
  labs(x = "Time since index's onset (days)",
       y = "Transmission reduction efficacy E(t)") +
  theme_minimal(base_size = 12, base_family = "sans") +
  theme(panel.grid.minor = element_blank())

ggsave("figures/antiviral_transmission_fit.png",
       p_fit, width = 5, height = 4, dpi = 500)
cat("Saved: figures/antiviral_transmission_reduction_fit.png\n")

# --------------------------------------------------------------
# Figure 2 — Diagnostics (trace + marginal posteriors)
# --------------------------------------------------------------
chain_df <- as.data.frame(chain_full) %>%
  mutate(sample = seq_len(n()))

param_levels <- c("Emax", "kappa", "tau")
...
ggsave("figures/antiviral_transmission_diagnostics.png",
       p_diag, width = 12, height = 8, dpi = 500)
cat("Saved: figures/antiviral_transmission_reduction_diagnostics.png\n")
