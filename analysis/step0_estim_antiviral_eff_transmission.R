...
# * Closed-form analytic integral used for interval data
################################################################################
...
# [5] MCMC settings (Stan is efficient, ~4000 iter is usually enough)
N_ITER    <- 4000
BURN_IN   <- 2000
CHAINS    <- 4
SEED      <- 42
...
parameters {
  // Conditionally active parameters
...
# Free parameter mapping for plots
free_idx   <- c()
free_names <- c()
...
cat("\n=============================================================\n")
cat(" Posterior Summary — Stan Fit (Transmission Reduction)\n")
cat("=============================================================\n")
...
# ============================================================
# 5. Plots (Blue theme for Transmission Reduction)
# ============================================================
...
for (dtype in c("pdf", "png")) {
  ...
  fname <- paste0("figures/antiviral_transmission_reduction_stan", fix_tag, ".", dtype)
  ...
  plot(ts, Ec, type = "n",
       ...
       ylab = "Transmission reduction efficacy E(t)",
  ...
  # Blue theme for Transmission Reduction
  polygon(c(ts, rev(ts)), c(cilo, rev(cihi)),
          col = rgb(0, 0, 1, 0.15), border = NA)
  lines(ts, Ec, lwd = 2, col = "blue")
  lines(ts, cilo, lwd = 1, col = "blue", lty = 2)
  lines(ts, cihi, lwd = 1, col = "blue", lty = 2)
