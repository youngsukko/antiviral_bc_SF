################################################################################
# Logistic Antiviral Efficacy Model — Infection Prevention (Household)
...
# ============================================================
OUTPUT_DIR <- "output"
CACHE_FILE <- file.path(OUTPUT_DIR, "antiviral_eff_estim_infection_hh.rds")
...
post_mean <- apply(chain_full, 2, median)  # posterior median
...
# Interval observations: dashed red horizontal line spanning [a, b]
obs_interval <- obs %>%
  filter(type == "interval") %>%
  mutate(a_clip = pmax(a, PLOT_T_RANGE[1]))

# Point observations: filled circle
obs_point <- obs %>%
  filter(type == "point") %>%
  mutate(t_plot = a)
...
p_fit <- ggplot() +
  ...
  # Interval observation: dashed red horizontal line spanning [a, b]
...
ggsave("figures/antiviral_infection_hh_fit.png",
       p_fit, width = 5, height = 4, dpi = 500)
cat("Saved: figures/antiviral_infection_hh_fit.png\n")
...
p_trace <- ggplot(long_df, aes(x = sample, y = value)) +
  ...
  geom_hline(data = post_mean_df, aes(yintercept = median),
...
p_hist <- ggplot(long_df, aes(x = value)) +
  ...
  geom_vline(data = post_mean_df, aes(xintercept = median),
...
ggsave("figures/antiviral_infection_hh_diagnostics.png",
       p_diag, width = 12, height = 8, dpi = 500)
cat("Saved: figures/antiviral_infection_hh_diagnostics.png\n")
